 ...
 * industry-news.js
 * ------------------------------------------------------------------
 * Pulls news articles from a published Google Sheet (as CSV) and
 * renders them into the Industry News page
 *
 * Each row needs "Published" set to "Yes" to appear on the live page.
 * Google's published CSV updates within a few minutes
 * ------------------------------------------------------------------
 */

const SHEET_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRdNr_ipjv66tGvmghTHWy5kY3MxZf2_vn_Y7oy9r0EfZXiLe8TomWLD4axFxbzo-F7kuWsWCYTNbzB/pub?gid=0&single=true&output=csv";
...
async function loadNews() {
  ...
  if (!SHEET_CSV_URL || SHEET_CSV_URL.includes("PASTE_YOUR")) {
    sectionsContainer.innerHTML =
      "<p>News feed isn't configured yet — add the published Google Sheet CSV URL in industry-news.js.</p>";
    return;
  }
  ...
  try {
    ...
    // newest first when a Date column is present
    rows.sort((a, b) => new Date(b.Date) - new Date(a.Date));

    if (rows.length === 0) {
      sectionsContainer.innerHTML = "<p>No news articles yet. Check back soon.</p>";
      tocList.innerHTML = "";
      return;
    }
    ...
    console.error("Failed to load news from Google Sheet:", err);
    sectionsContainer.innerHTML =
